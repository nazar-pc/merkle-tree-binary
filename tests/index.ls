...
test('merkle-tree-binary 1', (t) !->
	...
	t.same(lib.get_root([item], sha1), item)
	t.equal(lib.get_proof([item], item, sha1).length, 0)
	t.ok(lib.check_proof(item, new Uint8Array(0), item, sha1))
	t.notOk(lib.check_proof(item, new Uint8Array(1), item, sha1))
	t.end()
)
...
test('merkle-tree-binary 2', (t) !->
	...
	t.same(lib.get_root([item1, item2], sha1), root)
	t.same(lib.get_proof([item1, item2], item1, sha1), proof1)
	t.same(lib.get_proof([item1, item2], item2, sha1), proof2)
	t.ok(lib.check_proof(root, proof1, item1, sha1))
	t.ok(lib.check_proof(root, proof2, item2, sha1))
	t.notOk(lib.check_proof(root, proof2, item1, sha1))
	t.notOk(lib.check_proof(root, proof1, item2, sha1))
	t.end()
)
...
test('merkle-tree-binary 3', (t) !->
	...
	t.same(lib.get_root([item1, item2, item3], sha1), root)
	t.same(lib.get_proof([item1, item2, item3], item1, sha1), proof1)
	t.same(lib.get_proof([item1, item2, item3], item2, sha1), proof2)
	t.same(lib.get_proof([item1, item2, item3], item3, sha1), proof3)
	t.ok(lib.check_proof(root, proof1, item1, sha1))
	t.ok(lib.check_proof(root, proof2, item2, sha1))
	t.ok(lib.check_proof(root, proof3, item3, sha1))
	t.notOk(lib.check_proof(root, proof3, item1, sha1))
	t.notOk(lib.check_proof(root, proof2, item3, sha1))
	t.notOk(lib.check_proof(root, proof1, item2, sha1))
	t.end()
)
